...
public class MultiPlayer3Ctrl : CharacterBase
{

    private void Awake()
    {
        Bind(CharacterEvent.MULTI_INIT_PLAYER3_CARD,
            CharacterEvent.MULTI_HIT_PLAYER3_CARD,
            CharacterEvent.MULTI_CLEAR_PLAYER3_CARD);
    }
    ...
    public override void Execute(int eventCode, object message)
    {
        switch (eventCode)
        {
            case CharacterEvent.MULTI_INIT_PLAYER3_CARD:
                StartCoroutine(initPlayerCard(message as List<CardDto>));
                break;
            case CharacterEvent.MULTI_HIT_PLAYER3_CARD:
                getPlayerCard(message as CardDto);
                break;
            case CharacterEvent.MULTI_CLEAR_PLAYER3_CARD:
                clearPlayerCard();
                break;
            default:
                break;
