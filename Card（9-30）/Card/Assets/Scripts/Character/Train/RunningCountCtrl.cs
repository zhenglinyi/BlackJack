...
public class RunningCountCtrl : CharacterBase
{

    private void Awake()
    {
        Bind(CharacterEvent.RUNCOUNT_INIT_CARD,
            
            CharacterEvent.RUNCOUNT_CLEAR_CARD);
    }
    ...
    public override void Execute(int eventCode, object message)
    {
        switch (eventCode)
        {
            case CharacterEvent.RUNCOUNT_INIT_CARD:
                StartCoroutine(initPlayerCard(message as List<Card>));
                break;
            
            case CharacterEvent.RUNCOUNT_CLEAR_CARD:
                clearPlayerCard();
                break;
            default:
                break;
