...
public class MultiPLayer4Ctrl : CharacterBase
{

    private void Awake()
    {
        Bind(CharacterEvent.MULTI_INIT_PLAYER4_CARD,
            CharacterEvent.MULTI_HIT_PLAYER4_CARD,
            CharacterEvent.MULTI_CLEAR_PLAYER4_CARD);
    }
    ...
    public override void Execute(int eventCode, object message)
    {
        switch (eventCode)
        {
            case CharacterEvent.MULTI_INIT_PLAYER4_CARD:
                StartCoroutine(initPlayerCard(message as List<CardDto>));
                break;
            case CharacterEvent.MULTI_HIT_PLAYER4_CARD:
                getPlayerCard(message as CardDto);
                break;
            case CharacterEvent.MULTI_CLEAR_PLAYER4_CARD:
                clearPlayerCard();
                break;
            default:
                break;
