...
namespace AhpilyServer
{
    ...
    {
        public static byte[] DesKey = new byte[] { 1, 2, 3, 4, 8, 7, 6, 5 };
        ...
        public static byte[] EncodePacket(byte[] data)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                {
                    byte[] encryptData = DESEncrypt(data, DesKey, DesKey);
                    ////先写入长度
                    //bw.Write(data.Length);
                    ////再写入数据
                    //bw.Write(data);
                    bw.Write(encryptData.Length);
        ...
        public static byte[] DecodePacket(ref List<byte> dataCache)
        {
            ...
            //using (MemoryStream ms = new MemoryStream(decryptData))
            {
                using (BinaryReader br = new BinaryReader(ms))
                {
                    ...
                    byte[] decryptData = DESDecrypt(data, DesKey, DesKey);
                    //更新一下数据缓存
                    dataCache.Clear();
